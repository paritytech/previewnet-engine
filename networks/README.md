...
| `relay.fastRuntime` | Same as `relay.runtime`, but used when `PPN_PROFILE` is `local`. Needs a different `file`: `ppn fetch` skips a file it already has from the same release, so sharing one leaves the other variant's blob in place. |
...
| `parachains[].dotnsDispatcher` | Contract `pallet-dotns-gateway` dispatches into, written into this chain's state at bite time. Only a fork of a chain without Sudo needs it: `set_dispatcher_address` takes `RootOrWhitelist`, unreachable there, so `reserve_name` and `register_name` fail `DispatcherAddressNotSet`. |
| `parachains[].dotnsDeployer` | Ethereum wallet, or list of them, that deploys dotNS after the spawn. The bite endows each one's revive fallback account, because the deploy is signed by a wallet with no funds on a fork of a real chain. A signed transfer would work too, but only as a manual step repeated on every rebite. |
...
| dotNS dispatcher | `DotnsGateway::DispatcherAddress` unset, and the pallet itself absent until the upgrade lands | `set_dispatcher_address` takes `RootOrWhitelist`, so on a fork with no Sudo nothing can ever set it and `reserve_name` and `register_name` fail `DispatcherAddressNotSet` | the address the parachain's `dotnsDispatcher` names, written as an inject. Unverifiable (the live metadata has no such pallet) and guarded on the descriptor rather than `index.pallets.has`, which would be false at bite time; see `docs/FORK.md` |
