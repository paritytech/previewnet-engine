...
| `upgrades` | `{ "repo", "runtimes": { "<chain>": "<runtime name>" } }` — where the runtimes a fork of this network is upgraded to are published, and what each chain's is called there (`polkadot-fellows/runtimes` names its assets `<runtime>_runtime-v<spec>.compact.compressed.wasm`). Read by `make fetch-runtimes RUNTIMES=<tag or pr-N>` — release assets for a tag, build artifacts of the PR's head commit (one per runtime, named after it) for a pull request — which is what a sudo-less network needs before `make bite RUNTIMES=<ref>` can authorize them at import (docs/FORK.md). Networks with sudo upgrade after the spawn and need no table. |
