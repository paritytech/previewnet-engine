...
# A leftover eturnal from a previous run holds the ports. Its listeners only: a bare `lsof -i
...
for port in "$TURN_PORT" "$TURN_PROXY_PORT"; do
    lsof -ti "tcp:$port" -sTCP:LISTEN -a -c beam 2>/dev/null | xargs kill 2>/dev/null || true
done
