...
# How a bite lays the network out: CORES="<chain>=<n> ..." COLLATORS="<chain>=<n> ...". Fixed at
# bite time like UPGRADES, so a published bundle needs FRESH_BITE=1 to take them.
TOPOLOGY_FLAGS := $(foreach c,$(CORES),--cores $(c)) $(foreach c,$(COLLATORS),--collators $(c))
...
bite: build-spawner ensure-binaries
	@node $(CURDIR)/bin/ppn.mjs bite "$(FORK_DIR)" $(UPGRADE_FLAGS) $(TOPOLOGY_FLAGS)
